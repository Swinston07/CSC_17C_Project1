 ...
 *
 * Created on December 4, 2024, 4:30PM
 * This program is going to sort the hand of each player so that the player.
 * This will make it easier for the player to keep track of how many cards they
 * have of each suit. Limiting the number of incorrect choices.
 */
...
//    Player player4(p4Name);
    ...
    //Sort player 1's hand by the suit
    player1.sortHnd();
    
    //Set rest of players' hands
    for(Player& player : players){
        player.setHand(player1.deal());
        //Sort player's hand by the suit
        player.sortHnd();
    }
...
void Hand::sortHnd(){
    list<Card>::iterator pHand=listHand.begin(); //Set iterator to beginning of list
    list<Card>::iterator pHand2; //Make second iterator = first iterator
    list<Card>::iterator swapIt = listHand.end(); //Set to end of list
    list<Card>::iterator begin; //Use to advance second iterator forward 1
    //begin = advance(pHand2,1);
    advance(swapIt,-1); //Iterate to one less than end of list
    
    for(pHand = listHand.begin(); pHand!=swapIt; ++pHand){
        pHand2 = pHand;
        advance(pHand2,1);
        begin = pHand2;
        for(pHand2 = begin; pHand2 != listHand.end(); ++pHand2){
            if(pHand->getSuit() > pHand2->getSuit()){
                iter_swap(pHand,pHand2);
            }
        }
    }
}
